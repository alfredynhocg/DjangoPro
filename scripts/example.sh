...
do
   ...
   case $_DATABASES in
      postgres)
         ...
         ;;
      mysql)
         mysql -u root -p$PSQLPASSSYSTEM -e "DROP DATABASE $DATABASE;" 2> /dev/null
         mysql -u root -p$PSQLPASSSYSTEM -e "CREATE DATABASE $DATABASE;" 2> /dev/null
         create_env $_DATABASES $PORT_MYSQL $MYSQLUSER $PSQLPASSSYSTEM
         sleep 3
         // apt_install
         break
         ;;
